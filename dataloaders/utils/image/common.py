...
def center_crop_arr(pil_image, image_size,lq_image = None):
    ...
    if lq_image:
        lq_image = lq_image.resize(
        tuple(x for x in pil_image.size), resample=Image.BICUBIC
    )
    arr = np.array(pil_image)
    crop_y = (arr.shape[0] - image_size) // 2
    crop_x = (arr.shape[1] - image_size) // 2
    if lq_image:
        arr_lq = np.array(lq_image)
        return arr[crop_y : crop_y + image_size, crop_x : crop_x + image_size],arr_lq[crop_y : crop_y + image_size, crop_x : crop_x + image_size]
    else:
        return arr[crop_y : crop_y + image_size, crop_x : crop_x + image_size]


# https://github.com/openai/guided-diffusion/blob/main/guided_diffusion/image_datasets.py
def random_crop_arr(pil_image, image_size, min_crop_frac=0.8, max_crop_frac=1.0,lq_image = None):
